...
class Chart:
    ...
    def is_ready(self) -> bool:
        return len(self.time_series) > 100
    ...
    def make_figure(
        self, caption: str, label: str, from_dt: datetime = None, to_dt: datetime = None
    ) -> go.Figure:
        from_dt = from_dt or self.time_series[0]
        to_dt = to_dt or self.time_series[-1]
        chart_df = pd.DataFrame.from_records(
            [self.data[t] for t in self.time_series if from_dt <= t <= to_dt]
        )
