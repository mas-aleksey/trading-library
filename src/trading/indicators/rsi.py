...
class RSI(BaseIndicator):
    ...
    def add_trace(self, figure: go.Figure, chart_df: pd.DataFrame) -> None:
        ...
        figure.add_trace(
            go.Scatter(
                x=chart_df["time"],
                y=chart_df[f"RSI_ema_{self.period}"],
                mode="lines",
                name=f"RSI_ema_{self.ema_period}",
            ),
            row=2,
            col=1,
        )
        params = {"line_width": 1, "line_dash": "dash", "line_color": "black", "row": 2, "col": 1}
